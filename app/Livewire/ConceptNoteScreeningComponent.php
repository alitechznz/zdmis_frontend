...
class ConceptNoteScreeningComponent extends Component
{
    public $answers = [];
    public $comments = [];
    public $conceptNote = [];
    ...
    public function saveProjectDetails()
    {
        ...
        try {
            ...
            // dd($this->answers, $this->comments, $this->scores);

            foreach ($this->answers as $questionId => $answer) {

                $comment = $this->comments[$questionId] ?? null;
                $score = $this->scores[$questionId] ?? 0;


                // Ensure score is assigned correctly

                // dd($this->answer, $this->comment, $this->score);
                Screening::updateOrCreate(
                    ['projectquestion_id' => $questionId, 'conceptnote_id' => $this->cn_id, 'created_by' => auth()->user()->id],
                    ['answer' => $answer, 'comment' => $comment, 'score' => $score]
                );
            }

            $conceptNote = ConceptNote::find($this->cn_id);
            if ($conceptNote) {
                $conceptNote->process_status = 50;
                $conceptNote->return_status = 0;
                $conceptNote->save();
            }

            $this->dispatch('swal', title:'saved successfully!');
        } catch (\Exception $e) {
            // dd($e->getMessage());
    ...
    public function render()
    {
        ...
        return view('livewire.concept-note-screening-component', [
            'questions' => $questions,
