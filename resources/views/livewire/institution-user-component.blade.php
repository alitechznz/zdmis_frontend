<div x-data="data()" class="m-2">
        ...
        <div class="page-title">
            <div class="row">
                <div class="col-sm-6 p-0">
                    <h3>Manage Institution Users </h3>
                </div>
            </div>
        </div>
    </div>
                ...
                <div class="col-6">
                    <div class="input-group">
                        <input type="search" wire:model.live="search_keyword"
                               ...
                               placeholder="Search institution user...">
                    </div>
                </div>
                <div class="col-6">
                    <div class="float-end">
                        @can('add user institution')
                        <a href="#" class="btn btn-sm btn-primary" data-bs-toggle="modal"
                        data-bs-target="#modal-institutionuser" wire:click="create">
                ...
                <div class="col-sm-12 col-lg-12 col-xl-12">
                    <div class="table-responsive custom-scrollbar">
                        <table class="display border table table-bordered table-hover table-striped table-responsive custom-scrollbar-sm">
                            ...
                            <tbody>
                            @forelse ($institution_users as $institutionuser)
                                <tr>
                                    <td>{{ $institutionuser->full_name }}</td>
                                    <td>{{ $institutionuser->phone }}</td>
                                    <td>{{ $institutionuser->employmentID }}</td>
                                    <td>{{ $institutionuser->ministry?->name }}</td>
                                    <td>{{ $institutionuser->institution?->name }}</td>
                                    <td>{{ $institutionuser->address }}</td>
                                    <td style="display: flex; gap: 5px; justify-content: center;">
                                        @can('edit user institution')
                                        <a href="#" wire:click="edit({{ $institutionuser->id }})"
                                            class="btn btn-sm btn-success" data-bs-toggle="modal"
                                            data-bs-target="#modal-institutionuser">
                                             Edit </a>
                                        @endcan
                                        @can('delete user institution')
                                        <a href="#" class="btn btn-sm btn-danger"
                                        wire:click="deleteConfirm({{$institutionuser->id}})"
                                        data-bs-toggle="modal" data-bs-target="#deleteModalinstitutionuser">
                                         Delete </a>
                                        @endcan
    ...
    <!-- Modal Content -->
    <div class="modal fade" wire:ignore.self id="modal-institutionuser" tabindex="-1" role="dialog"
                ...
                <div class="modal-body">
                    <div class="row">
                        ...
                        <div class="mb-4 col-md-6 col-sm-6 col-lg-6">
                            ...
                            <select wire:model="ministry" class="form-select @error("ministry") is-invalid @enderror"
                                    id="ministry">
    ...
    <!-- Delete Modal -->
    <div wire:ignore.self class="modal fade" id="deleteModalinstitutionuser" data-backdrop="false" tabindex="-1"
    ...
    @push('scripts')
        <script>
            document.addEventListener('livewire:initialized', () => {
                @this.
                on('closeModal', (event) => {
                    $('#modal-institutionuser').modal('hide')
                });
            });
        </script>
    @endpush
