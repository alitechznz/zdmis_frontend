<div x-data="data()" class="m-2">
        ...
        <div class="card-body">
            <div class="row mb-3">
                ...
                <div class="col-6">
                    <div class="float-end">
                        @can('add region')
                        <a href="#" class="btn btn-sm btn-primary" data-bs-toggle="modal" data-bs-target="#modal-region" wire:click='create'
            ...
            <div class="table-responsive">
                <table class="table table-bordered table-sm table-hover table-striped table-responsive custom-scrollbar-sm">
                    ...
                    <tr class="text-capitalize">
                        <th @click="sortByColumn" class="cursor-pointer select-none">Region Name <span class="float-end text-secondary">&#8645;</span>
                    ...
                    @forelse ($regions as $region)
                        <tr>
                            <td>{{ $region->name }}</td>
                            <td><span class="badge badge-light-success">{{ ucfirst($region->status) }}</span></td>
                            <td style="display: flex; gap: 5px;">
                                @can('edit region')
                                <a href="#" wire:click="edit({{ $region->id }})"
                                    class="btn btn-sm btn-success" data-bs-toggle="modal" data-bs-target="#modal-region">
                                     Edit </a>
                                @endcan

                                @can('delete region')
                                <a href="#" class="btn btn-sm btn-danger" wire:click="deleteConfirm({{$region->id}})"
                                    data-bs-toggle="modal" data-bs-target="#deleteModalregion">
                                     Delete </a>
                                @endcan
    ...
    <!-- Modal Content -->
    <div class="modal fade" wire:ignore.self id="modal-region" tabindex="-1" role="dialog" aria-labelledby="modal-default" aria-hidden="true">
        <div class="modal-dialog modal-dialog-centered" role="document">
            <div class="modal-content">
                <div class="modal-header">
                    <h2 class="h6 modal-title">@if($update) Update @else Add @endif Region</h2>
                ...
                <div class="modal-body">
                    <div  class="row">
                        <div class="mb-4">
                            ...
                            <input type="text" wire:model="name" class="form-control @error("name") is-invalid @enderror" id="name" placeholder="Enter Region Name">
                            @error("name")
    ...
    <!-- Delete Modal -->
    <div wire:ignore.self class="modal fade" id="deleteModalregion" data-backdrop="false" tabindex="-1" role="dialog" aria-labelledby="deleteModalLabel" aria-hidden="true">
